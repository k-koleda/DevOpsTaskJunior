Configuration TargetTest {
	...
	Node "localhost" {
		...
		Script DownloadNet4.5.2 {
			SetScript = {
				if([System.IO.File]::Exists($using: Net452Loc) - ne $true - or(Get - FileHash $using: Net452Loc | % {
						$_ - match $using: hashNet452
					}) - ne $true) {
					try {
			...
			GetScript = {
				if(Get - ItemProperty - Path 'HKLM:\SOFTWARE\Microsoft\NET Framework Setup\NDP\v4\Full' | % {
						$_ - match '379893'
					}) {
		...
		Script DownloadZip {
			...
			TestScript = {
				if([System.IO.File]::Exists($using: ZipLoc)) {
					if(Get - FileHash $using: ZipLoc | % {
							$_ - match $using: hashZip
						}) {
						return $true
					} else {
						return $false
		...
		Script FindError {
			...
			TestScript = {
				if((Get - Content "$using:SiteLoc\Web.config") - match '<system.web.>') {
					return $False
				} else {
					return $True
				}
