configuration TargetTest {
  ...
  node "localhost" {
    ...
    script DownloadNet4.5.2 {
      SetScript = {
        if ([System.IO.File]::Exists($using:Net452Loc) -ne $true -or (Get-FileHash $using:Net452Loc | ForEach-Object { $_ -match $using:hashNet452 }) -ne $true)
        {
          try {
      ...
      TestScript = {
        Get-ItemProperty -Path 'HKLM:\SOFTWARE\Microsoft\NET Framework Setup\NDP\v4\Full' | ForEach-Object { $_ -match '379893' }
      }

      GetScript = {
        if (Get-ItemProperty -Path 'HKLM:\SOFTWARE\Microsoft\NET Framework Setup\NDP\v4\Full' | ForEach-Object { $_ -match '379893' })
        {
    ...
    script DownloadZip {
      ...
      TestScript = {
        ...
        {
          if (Get-FileHash $using:ZipLoc | ForEach-Object { $_ -match $using:hashZip })
          { return $true }
          else
          { return $false } }
        else
